...
def load_data():
    ...
    term_doc_matrix = pd.read_csv('term_doc_matrix.csv', index_col=0)
    return train_df, term_doc_matrix


train_df, term_doc_matrix = load_data()

# User input query
query = st.text_input("Enter your query:")

if query:
    # Split the query into words and lower them
    query_words = query.lower().split()

    # Find the documents (columns) in term-doc-matrix that contain the query words
    matched_docs = pd.DataFrame()

    for word in query_words:
        if word in term_doc_matrix.index:
            matched_docs = pd.concat(
                [matched_docs, term_doc_matrix.loc[word]], axis=1)

    if not matched_docs.empty:
        # Sum word counts across all query words and sort documents by their count
